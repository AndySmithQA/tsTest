...
// console.log(useLocation());
    ...
    const [soldProperty, setSoldProperty] = useState({ buyerId: '', address: '', id: '' });
    const [loadingProperties, setLoadingProperties] = useState(false);
    const [saving, setSaving] = useState(false);

    const buyerSelectorId = "buyerSelector";

    const propertyListReducer = (state: any[], action: { type: string, payload: any }) => {
        switch (action.type) {
            case "ADD":
    ...
    const propertyAddHandler = (newProperty: any) => {
        setSaving(true);
    ...
    const propertyUpdateHandler = (propertyToUpdate: any, newStatus: string) => {
    ...
    const iconClassForStatus = (propertyStatus: string) => {
    ...
    return (
        <>
            ...
            <ul>
                {
                    properties.filter((property: any) => property.sellerId === seller.id).length === 0 && !loadingProperties ?
                        <li>
                            <div className="message alert alert-info" role="alert">
                        ...
                        :
                        properties.filter((property: any) => property.sellerId === seller.id).map((property: any) => (
                            <li key={property.id}>
