...
const Booking = () => {
    ...
    const bookingListReducer = (state: Booking[], action: BookingAction): Booking[] => {
        switch (action.type) {
            case "ADD":
    ...
    const bookingAddHandler = (buyerId: number, bookingDate: string) => {
        if (bookings.filter(booking => booking.propertyId === property.id && new Date(booking.date) === new Date(bookingDate)).length) {
        ...
        let newBooking = {
            "buyerId":  buyerId,
            "buyerFirstName": buyers.find(buyer => buyer.id === buyerId)?.firstName || "",
            "buyerSurname": buyers.find(buyer => buyer.id === buyerId)?.surname || "",
    ...
    const bookingRemoveHandler = (bookingId: number) => {
        ...
        fetch("http://localhost:8080/booking/" + bookingId, {
            ...
            dispatch({type: "REMOVE", payload: bookingId});
        });
    };

    const iconClassForPropertyType = (propertyType: string) => {
        switch (propertyType) {
            case "DETACHED" :
                    ...
                    bookings.filter(booking => booking.propertyId === property.id).length === 0 && !loadingBookings ?
                        ...
                        .map(booking => (
                            <li key={booking.id}>
                                ...
                                <b>{booking.buyerFirstName}&nbsp;{booking.buyerSurname}</b>
